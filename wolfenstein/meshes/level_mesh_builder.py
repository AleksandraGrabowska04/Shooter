...
# turns level_map into mesh
# checks what is a renderable wall, generates only visible walls
# counts AO, decides about uv flip, builds quads from triangles
# flip is needed for correct Ambient Occlusion calculations
            ...
            for z in range(self.map.depth):
                ...
                if pos_not_in_wall_map := (x, z) not in self.map.wall_map:
                    ...
                    if (x, z) in self.map.floor_map:
                        ...
                        face_id = 0
                        # format matching designed VBO, those values need to match with shaders
                        # here four points are specified and thus two triangles = one quad
