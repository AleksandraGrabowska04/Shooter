    ...
    def draw_calibration_visualization(self, frame: np.ndarray, landmarks: Dict,
        ...
        try:
            ...
            if calibration.is_calibrated and calibration.reference_position:
                ...
                reference_screen = (
                    int(reference_pos[0]), int(reference_pos[1]))
                ...
                if distance > max_distance:
                    ...
                    current_screen = (
                        int(current_wrist[0]), int(current_wrist[1]))
                    marker_color = (0, 0, 255)  # Red

                # Draw current position with appropriate color
                cv2.circle(frame, current_screen, 8,
                           marker_color, -1)    # Filled
                cv2.circle(frame, current_screen, 12,
                           marker_color, 2)    # Outline

                # Draw reference point (larger, green)
                cv2.circle(frame, reference_screen, 10,
                           (0, 255, 0), -1)  # Green filled
                cv2.circle(frame, reference_screen, 15,
                           (0, 255, 0), 3)   # Green outline

                # Draw max distance circle (faint white)
                cv2.circle(frame, reference_screen, int(
                    max_distance), (100, 100, 100), 1)

                # Draw line connecting positions
                cv2.line(frame, reference_screen, current_screen,
                ...
                cv2.circle(frame, current_screen, 8,
                           (0, 0, 255), -1)    # Red filled
                cv2.circle(frame, current_screen, 12,
                           (0, 0, 255), 2)    # Red outline

                # Show calibration prompt (activation gesture)
                text_renderer.draw_text_with_outline(
                    frame,
                    "Activate to calibrate",
