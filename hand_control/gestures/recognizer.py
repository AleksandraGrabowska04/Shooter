...
from .strategies import (
    FistDetectionStrategy, ShootDetectionStrategy, PositionDetectionStrategy,
...
class GestureRecognizer(IGestureRecognizer):
    ...
    def __init__(self, config: ApplicationConfig):
        ...
        self.fist_detector = FistDetectionStrategy(config)
        self.shoot_detector = ShootDetectionStrategy(config)
    ...
    def detect_gestures(self, landmarks: Optional[dict]) -> List[GestureResult]:
        ...
        try:
            ...
            fist_gestures = self.fist_detector.detect(processed_landmarks, self.calibration_data)
            all_gestures.extend(fist_gestures)
            
            all_gestures.extend(
                self.shoot_detector.detect(processed_landmarks, self.calibration_data)
            )
            all_gestures.extend(
                self.position_detector.detect(processed_landmarks, self.calibration_data)
            )
            all_gestures.extend(
                self.orientation_detector.detect(processed_landmarks, self.calibration_data)
            )
            all_gestures.extend(
                self.motion_detector.detect(processed_landmarks, self.calibration_data)
            )
    ...
    def calibrate(self, landmarks: dict) -> bool:
        ...
        try:
            ...
            # Require multiple frames for stable calibration
            self.calibration_frame_count += 1
            if self.calibration_frame_count < self.config.gestures.calibration_frames:
                return False
            
            # Store reference position
            wrist = processed_landmarks["WRIST"]
            self.calibration_data.reference_position = (wrist[0], wrist[1], wrist[2])
    ...
    def reset_calibration(self) -> None:
        ...
        self.motion_detector.motion_history.clear()
