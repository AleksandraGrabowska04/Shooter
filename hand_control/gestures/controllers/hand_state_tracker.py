...
class HandStateTracker:
    ...
    def __init__(self, logger: ILogger):
        ...
        self._fist_buffer_size = 7  # Require 7 consistent frames before changing state (increased for maximum rotation stability)
    ...
    def update_hand_state(self, current_state: HandState) -> bool:
        ...
        if len(self._fist_buffer) >= self._fist_buffer_size:
            ...
            fist_count = sum(1 for state in self._fist_buffer if state == HandState.FIST)
            open_count = sum(1 for state in self._fist_buffer if state == HandState.OPEN)
            ...
            if self._confirmed_fist_state == HandState.FIST:
                ...
                if open_count > (self._fist_buffer_size * 0.8):  # 80% threshold for maximum stability
                ...
                if fist_count >= (self._fist_buffer_size * 0.3):  # 30% threshold for switching to fist
        ...
        if self._debug_mode and buffered_state != self._last_hand_state:
            self.logger.info(f"[TRANSITION] HAND STATE: {self._last_hand_state.value if self._last_hand_state else 'None'} -> {buffered_state.value} (buffer: {[s.value for s in self._fist_buffer]})")
        
        # Reset calibration when transitioning from fist to open
        if (self._confirmed_fist_state == HandState.FIST and 
            buffered_state == HandState.OPEN and 
            self._reset_calibration_callback):
            self.logger.info("[RESET] Calibration: fist -> open")
            self._reset_calibration_callback()
